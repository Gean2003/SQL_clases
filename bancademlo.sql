...
select accounts.account_no as cuenta, accounts.client_id as id_cliente, account_types."name" as tipo_de_cuenta  from accounts  right join account_types on accounts.id = account_types.id;

select transactions.id as transaccion, transactions.amount as monto, transactions_types.name as "tipo de transaccion"from transactions right join transactions_types on transactions.id = transactions_types.id;  
...
create view v_accounts_types as select accounts.account_no as cuenta, accounts.client_id as id_cliente, account_types."name" as tipo_de_cuenta  from accounts  right join account_types on accounts.id = account_types.id;

create view v_transactions_detail as select transactions.id as transaccion, transactions.amount as monto, transactions_types.name as "tipo de transaccion"from transactions right join transactions_types on transactions.id = transactions_types.id;  
